...
const slides = [
  {
    id: 'welcome',
    title: `Bem-vindo ao ${BRAND_NAME}`,
    description:
      'Seu assistente de inteligência clínica estética. Análise com IA, planejamento digital do sorriso e protocolos personalizados em minutos.',
    illustration: (
  ...
  },
  {
    id: 'how-it-works',
    title: 'Como funciona',
    description: 'Três passos simples para um protocolo completo.',
    illustration: (
      <div className="flex items-center justify-center gap-3">
        {[
          { icon: Camera, label: 'Foto', color: 'bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400' },
          { icon: Sparkles, label: 'IA', color: 'bg-primary/10 text-primary' },
          { icon: FileText, label: 'Protocolo', color: 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400' },
        ].map((step, i) => (
  ...
  },
  {
    ...
    title: 'Comece agora',
    description:
      'Explore um caso exemplo para ver o sistema em ação, ou crie sua primeira avaliação.',
    illustration: null,
  },
];
        ...
        <div className="px-6 pt-8 pb-6 text-center space-y-5">
          ...
          <h2 className="text-xl font-semibold font-display">
            {slide.id === 'welcome' ? (
              <>
                Bem-vindo ao{' '}
                <span className="text-gradient-gold">{BRAND_NAME}</span>
              </>
            ) : (
              slide.title
            )}
