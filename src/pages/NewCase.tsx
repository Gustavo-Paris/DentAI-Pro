...
export default function NewCase() {
  ...
  // Helper to check if DSD suggestion means "keep as is" (no treatment needed)
  const isDSDSuggestionToKeep = (proposedChange: string, currentIssue: string): boolean => {
    const lowerProposed = proposedChange.toLowerCase();
    const lowerIssue = currentIssue.toLowerCase();

    // Keywords that indicate the tooth should be kept/maintained
    const keepKeywords = [
      'manter', 'preservar', 'conservar', 'referência', 'guia',
      'integridade', 'sem intervenção', 'não necessita', 'excelente',
      'ideal', 'perfeito', 'sem alteração'
    ];

    // Check if proposed change suggests keeping the tooth
    if (keepKeywords.some(kw => lowerProposed.includes(kw))) {
      return true;
    }

    // Check if current issue indicates the tooth is already good
    if (lowerIssue.includes('excelente') || lowerIssue.includes('referência') ||
        lowerIssue.includes('ideal') || lowerIssue.includes('perfeito')) {
      return true;
    }
  ...
  // Helper to infer treatment indication from DSD suggestion
  const inferTreatmentFromDSD = (proposedChange: string): TreatmentType | null => {
    const lower = proposedChange.toLowerCase();

    // Check if this is a "keep as is" suggestion - return null (no treatment)
    const keepKeywords = [
      'manter', 'preservar', 'conservar', 'referência', 'guia',
      'integridade', 'sem intervenção', 'não necessita'
    ];
    if (keepKeywords.some(kw => lower.includes(kw))) {
      return null; // No treatment needed
    }

    // Check for porcelain/facet indicators
    if (lower.includes('faceta') || lower.includes('laminado') || lower.includes('porcelana') ||
        lower.includes('cerâmica') || lower.includes('veneer')) {
      return 'porcelana';
    }

    // Check for crown indicators
    if (lower.includes('coroa') || lower.includes('crown')) {
      return 'coroa';
    }

    // Check for implant indicators
    if (lower.includes('implante') || lower.includes('extração')) {
      return 'implante';
    }

    // Check for endodontics indicators
    if (lower.includes('canal') || lower.includes('endodont')) {
      return 'endodontia';
    }

    // Default to resina for conservative treatments
    return 'resina';
  };

  // Merge DSD suggestions with detected teeth
  // DSD has priority for aesthetic decisions - if DSD says "keep", remove from treatment list
  const mergeDetectedTeethWithDSD = (
    detectedTeeth: DetectedTooth[],
    dsdSuggestions: { tooth: string; current_issue: string; proposed_change: string; treatment_indication?: TreatmentType }[]
  ): DetectedTooth[] => {
    // Create a map of DSD suggestions for quick lookup
    const dsdMap = new Map(dsdSuggestions.map(s => [s.tooth, s]));

    // First, filter out teeth that DSD says should be kept (no treatment)
    const teethToKeep = new Set<string>();
    for (const suggestion of dsdSuggestions) {
      if (isDSDSuggestionToKeep(suggestion.proposed_change, suggestion.current_issue)) {
        teethToKeep.add(suggestion.tooth);
        console.log(`[DSD Merge] Tooth ${suggestion.tooth} marked as "keep" - will remove from treatment list`);
      }
    }
    ...
    const existingToothNumbers = new Set(mergedTeeth.map(t => t.tooth));

    // Process DSD suggestions
    for (const suggestion of dsdSuggestions) {
      // Skip teeth that should be kept
      if (teethToKeep.has(suggestion.tooth)) {
        continue;
      }

      // Use treatment_indication from DSD if available, otherwise infer
      const treatmentFromDSD = suggestion.treatment_indication || inferTreatmentFromDSD(suggestion.proposed_change);

      // Skip if treatment is null (means keep as is)
      if (treatmentFromDSD === null) {
        console.log(`[DSD Merge] Skipping tooth ${suggestion.tooth} - inferred as "keep"`);
        continue;
      }

      if (!existingToothNumbers.has(suggestion.tooth)) {
        // This tooth is in DSD but not in photo analysis - add it
        const toothNum = parseInt(suggestion.tooth);
        const isUpper = toothNum >= 10 && toothNum <= 28;
        const isAnteriorTooth = ['11', '12', '13', '21', '22', '23', '31', '32', '33', '41', '42', '43'].includes(suggestion.tooth);

        const newTooth: DetectedTooth = {
          ...
          depth: null,
          priority: 'média', // DSD suggestions are aesthetic, so medium priority
          notes: `DSD: ${suggestion.current_issue} → ${suggestion.proposed_change}`,
          treatment_indication: treatmentFromDSD,
          indication_reason: suggestion.proposed_change,
        };

        mergedTeeth.push(newTooth);
        existingToothNumbers.add(suggestion.tooth);
        console.log(`[DSD Merge] Added tooth ${suggestion.tooth} from DSD with treatment: ${treatmentFromDSD}`);
      } else {
        // Tooth exists in both - DSD has priority for treatment indication
        const existingTooth = mergedTeeth.find(t => t.tooth === suggestion.tooth);
        if (existingTooth) {
          // DSD has priority - update treatment indication
          if (suggestion.treatment_indication) {
            existingTooth.treatment_indication = suggestion.treatment_indication;
            existingTooth.indication_reason = suggestion.proposed_change;
            console.log(`[DSD Merge] Updated tooth ${suggestion.tooth} treatment to: ${suggestion.treatment_indication} (DSD priority)`);
          } else if (treatmentFromDSD) {
            existingTooth.treatment_indication = treatmentFromDSD;
            existingTooth.indication_reason = suggestion.proposed_change;
            console.log(`[DSD Merge] Updated tooth ${suggestion.tooth} treatment to: ${treatmentFromDSD} (inferred from DSD)`);
          }

          // Add DSD notes if not already present
          if (!existingTooth.notes?.includes('DSD:')) {
            existingTooth.notes = existingTooth.notes
              ? `${existingTooth.notes} | DSD: ${suggestion.proposed_change}`
              : `DSD: ${suggestion.proposed_change}`;
          }
        }
      }
    }

    console.log(`[DSD Merge] Final merged teeth count: ${mergedTeeth.length}`);

    // Sort by tooth number for consistent display
    return mergedTeeth.sort((a, b) => {
      const numA = parseInt(a.tooth) || 0;
      const numB = parseInt(b.tooth) || 0;
      return numA - numB;
    });
  };
  ...
  const handleDSDComplete = (result: DSDResult | null) => {
    ...
    // Merge DSD suggestions with detected teeth
    if (result?.analysis && analysisResult) {
      const mergedTeeth = result.analysis.suggestions && result.analysis.suggestions.length > 0
        ? mergeDetectedTeethWithDSD(
            analysisResult.detected_teeth || [],
            result.analysis.suggestions
          )
        : analysisResult.detected_teeth || [];

      // Keep all warnings from photo analysis - let the dentist decide
      // DSD observations are complementary, not replacement for warnings
      const warnings = analysisResult.warnings || [];

      // Update analysisResult with merged teeth (keep original warnings)
      setAnalysisResult(prev => prev ? {
        ...prev,
        detected_teeth: mergedTeeth,
        warnings: warnings,
      } : null);

      // Update selected teeth to include all merged teeth
      setSelectedTeeth(mergedTeeth.map(t => t.tooth));

      // Update tooth treatments with the merged data
      const updatedTreatments: Record<string, TreatmentType> = { ...toothTreatments };
      mergedTeeth.forEach(t => {
        if (!updatedTreatments[t.tooth]) {
          updatedTreatments[t.tooth] = t.treatment_indication || 'resina';
        }
      });
