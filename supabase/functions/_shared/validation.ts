...
// CaseData validation for analyze-case
...
export function validateCaseData(data: unknown): ValidationResult<CaseData> {
  if (!data || typeof data !== "object") {
    return { success: false, error: "Dados inválidos" };
  }
  ...
  // Required UUID field
  if (!isUUID(obj.caseId)) {
    return { success: false, error: "ID do caso inválido" };
  }

  // Required number field
  if (typeof obj.patientAge !== "number" || obj.patientAge < 0 || obj.patientAge > 150) {
    return { success: false, error: "Idade do paciente inválida" };
  }

  // Required string fields
  if (!isValidTooth(obj.toothNumber)) {
    return { success: false, error: "Número do dente inválido" };
  }

  if (!isString(obj.toothRegion, 100)) {
    return { success: false, error: "Região do dente inválida" };
  }

  if (!isString(obj.cavityClass, 50)) {
    return { success: false, error: "Classe de cavidade inválida" };
  }

  if (!isString(obj.restorationSize, 50)) {
    return { success: false, error: "Tamanho de restauração inválido" };
  }

  if (!isString(obj.substrate, 100)) {
    return { success: false, error: "Substrato inválido" };
  }

  if (!isBoolean(obj.hasBruxism)) {
    return { success: false, error: "Campo bruxismo inválido" };
  }

  if (!isString(obj.aestheticLevel, 50)) {
    return { success: false, error: "Nível estético inválido" };
  }

  if (!isValidVitaShade(obj.toothShade)) {
    return { success: false, error: "Cor VITA inválida" };
  }

  if (!isBoolean(obj.needsStratification)) {
    return { success: false, error: "Campo estratificação inválido" };
  }
  ...
  return {
    ...
    data: {
      ...
      patientAge: obj.patientAge as number,
      toothNumber: obj.toothNumber as string,
      toothRegion: obj.toothRegion as string,
      cavityClass: obj.cavityClass as string,
      restorationSize: obj.restorationSize as string,
      substrate: obj.substrate as string,
      hasBruxism: obj.hasBruxism as boolean,
      aestheticLevel: obj.aestheticLevel as string,
      toothShade: obj.toothShade as string,
      needsStratification: obj.needsStratification as boolean,
      clinicalNotes: obj.clinicalNotes as string | undefined,
    },
  };
}
