...
export interface SessionDetailProps {
  ...
  onCompleteAll?: () => void
  onDelete?: () => void
  onViewProtocol?: (evalId: string) => void
  onAddTeeth?: () => void
  ...
  return (
    <div className="section-glow-bg relative min-h-screen p-6 sm:p-8 space-y-6">
      ...
      <nav className="flex items-center gap-1">
        <button
          onClick={() => onBack?.()}
          className="text-sm text-primary font-medium hover:underline focus-visible:ring-2 focus-visible:ring-ring rounded"
        >
          Avaliacoes
        </button>
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-4 text-xs text-muted-foreground mt-1">
                <span className="flex items-center gap-1">
                  ...
                  {new Date(header.created_at).toLocaleDateString('pt-BR')}
                </span>
                <span className="flex items-center gap-1">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          onClick={() => onShare?.()}
          className="flex items-center gap-1.5 text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-ring rounded-lg px-3 py-2 border border-border text-foreground hover:bg-muted"
        >
        ...
        <button
          onClick={() => onDelete?.()}
          className="flex items-center gap-1.5 text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-ring rounded-lg px-3 py-2 text-destructive hover:bg-destructive/10"
        >
          ...
          <div key={group.treatmentType}>
            {group.evaluations.length > 1 && (
              <div className="flex items-center justify-between px-3 py-2 bg-muted/30 rounded-lg mb-2">
                ...
                <button
                  onClick={() =>
                    onViewProtocol?.(group.evaluations[0]?.id ?? '')
                  }
                  className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors focus-visible:ring-2 focus-visible:ring-ring rounded px-1.5 py-1"
                >
            ...
            {group.evaluations.map((ev) => {
              ...
              const borderColor = isCompleted
                ? 'var(--color-success)'
                : (TREATMENT_COLOR_VARS[ev.treatment_type] ??
                  'var(--color-primary)')

              return (
                <div
                  key={ev.id}
                  onClick={() => onViewProtocol?.(ev.id)}
      ...
      <div className="rounded-xl p-4 border border-primary/20 bg-primary/5 mt-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-foreground">
              Adicione mais dentes
            </p>
            <p className="text-xs text-muted-foreground">
              2 dentes pendentes nesta sessao
            </p>
          </div>
          <button
            onClick={() => onAddTeeth?.()}
            className="text-sm bg-primary text-primary-foreground rounded-lg px-3 py-1.5 btn-press focus-visible:ring-2 focus-visible:ring-ring transition-colors font-medium"
          >
            Adicionar
          </button>
        </div>
