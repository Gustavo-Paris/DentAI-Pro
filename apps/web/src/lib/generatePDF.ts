...
export async function generateProtocolPDF(data: PDFData): Promise<void> {
  ...
  if (data.clinicName) {
    ...
    addText('ToSmile.ai - Protocolo de Restauracao Estetica', headerTextStartX, 20, { fontSize: 9, color: [42, 157, 143] });
  } else {
    addText('ToSmile.ai', headerTextStartX, 14, { fontSize: 20, fontStyle: 'bold', color: [255, 255, 255] });
