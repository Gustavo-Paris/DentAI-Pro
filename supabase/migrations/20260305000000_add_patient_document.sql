-- Add patient_document jsonb column to evaluations_raw table
-- (evaluations is a view over evaluations_raw with PHI decryption)
ALTER TABLE public.evaluations_raw
  ADD COLUMN IF NOT EXISTS patient_document jsonb DEFAULT NULL;

COMMENT ON COLUMN public.evaluations_raw.patient_document IS 'AI-generated patient-facing document (explanation, post-op, diet, TCLE)';

-- Recreate the evaluations view to include patient_document
...
-- Recreate INSTEAD OF UPDATE trigger to include patient_document
...
-- Recreate INSTEAD OF INSERT trigger to include patient_document
