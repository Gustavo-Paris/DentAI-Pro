 ...
 *
 * Standardized error component for pages and error boundaries.
...
export type ErrorVariant = 'default' | 'student' | 'creator' | 'admin';
...
/**
 * ErrorState - Standardized error component for pages and error boundaries
 *
 * Provides consistent error UI with:
 * - Theme-aware styling (works with student, creator, admin themes)
 * - Optional error logging via onError callback
...
export const ErrorState = forwardRef<HTMLDivElement, ErrorStateProps>(
  function ErrorState(
    ...
    // Resolve icons
    const AlertTriangleIcon = resolveIcon('alert-triangle');
    const RefreshIcon = resolveIcon('refresh');
      ...
      >
        <div className="max-w-md w-full space-y-6 text-center">
          ...
          >
            {AlertTriangleIcon && (
              <AlertTriangleIcon
                className={cn('w-8 h-8', styles.iconColor)}
                aria-hidden="true"
              />
            )}
          </div>
            ...
            >
              ...
              {error.stack && (
                <details className="mt-2">
                  ...
                  <pre
                    className={cn(
                      'text-xs mt-2 overflow-auto max-h-40',
                      styles.errorText
                    )}
                  >
                    {error.stack}
