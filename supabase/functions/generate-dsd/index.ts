...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // existingAnalysis is now REQUIRED — analysis is done by analyze-dental-photo
    if (!existingAnalysis) {
      return createErrorResponse("existingAnalysis é obrigatório. A análise DSD deve ser feita via analyze-dental-photo.", 400, corsHeaders);
    }

    // Validate ownership if evaluationId provided
    if (evaluationId) {
    ...
    const analysis: DSDAnalysis = existingAnalysis;
