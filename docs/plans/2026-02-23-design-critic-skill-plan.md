# Design Critic Skill — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a monolithic `design-critic` skill that acts as an extremely critical senior designer, evaluating the web application across 5 pillars (Visual Consistency, Information Hierarchy, Interaction Quality, Spatial Design, Polish & Craft), taking screenshots via Playwright, and auto-fixing all issues found.

**Architecture:** Single SKILL.md file in `~/.claude/skills/design-critic/` following the established skill format (YAML frontmatter + markdown body). The skill instructs the agent to: (1) map all pages and design tokens, (2) capture screenshots via Playwright MCP, (3) analyze code against 5 pillar checklists, (4) visually analyze screenshots, (5) generate scored report, (6) auto-fix all findings P0 through P2.

**Tech Stack:** Claude Code skill system (SKILL.md), Playwright MCP tools for screenshots, Tailwind CSS semantic tokens, PageShell design system.

**Design doc:** `docs/plans/2026-02-23-design-critic-skill-design.md`
...
**Files:**
- Create: `~/.claude/skills/design-critic/SKILL.md`

**Step 1: Create directory**

```bash
mkdir -p ~/.claude/skills/design-critic
```

**Step 2: Write SKILL.md with frontmatter and overview**

Write the file `~/.claude/skills/design-critic/SKILL.md` with:
...
```bash
cat ~/.claude/skills/design-critic/SKILL.md | head -15
```

Expected: File exists with frontmatter and overview header.

**Step 4: Commit**

```bash
git add ~/.claude/skills/design-critic/SKILL.md
git commit -m "feat: create design-critic skill scaffold"
```

---

### Task 2: Add operation modes and execution flow

**Files:**
- Modify: `~/.claude/skills/design-critic/SKILL.md`

**Step 1: Append operation modes section**

Append to the SKILL.md after the overview:
...
```bash
grep "Modos de Operacao" ~/.claude/skills/design-critic/SKILL.md
grep "RECONNAISSANCE" ~/.claude/skills/design-critic/SKILL.md
```
...
```bash
git add ~/.claude/skills/design-critic/SKILL.md
git commit -m "feat(design-critic): add operation modes and execution flow"
```

---

### Task 3: Add the 5-pillar evaluation framework

**Files:**
- Modify: `~/.claude/skills/design-critic/SKILL.md`

**Step 1: Append the framework section**
...
```bash
git add ~/.claude/skills/design-critic/SKILL.md
git commit -m "feat(design-critic): add 5-pillar evaluation framework with checklists"
```

---

### Task 4: Add anti-patterns, premium patterns, and hard rules

**Files:**
- Modify: `~/.claude/skills/design-critic/SKILL.md`

**Step 1: Append anti-patterns and premium patterns**
...
```bash
git add ~/.claude/skills/design-critic/SKILL.md
git commit -m "feat(design-critic): add hard rules, anti-patterns, and premium patterns"
```

---

### Task 5: Add report template and cross-skill integration

**Files:**
- Modify: `~/.claude/skills/design-critic/SKILL.md`

**Step 1: Append report template and integration sections**
...
```bash
git add ~/.claude/skills/design-critic/SKILL.md
git commit -m "feat(design-critic): add report template, cross-skill integration, and reference context"
```

---

### Task 6: Verify complete skill and run smoke test

**Files:**
- Read: `~/.claude/skills/design-critic/SKILL.md`

**Step 1: Verify file structure**

```bash
wc -l ~/.claude/skills/design-critic/SKILL.md
```
...
```bash
head -5 ~/.claude/skills/design-critic/SKILL.md
```

Expected: Valid YAML frontmatter with `name: design-critic`.

**Step 3: Verify all sections present**

```bash
grep "^## " ~/.claude/skills/design-critic/SKILL.md
```
...
**Step 4: Verify triggers in description**

```bash
grep "design audit" ~/.claude/skills/design-critic/SKILL.md
grep "design review" ~/.claude/skills/design-critic/SKILL.md
grep "design critic" ~/.claude/skills/design-critic/SKILL.md
```

Expected: All triggers present in description frontmatter.

**Step 5: Final commit with design doc**

```bash
git add docs/plans/2026-02-23-design-critic-skill-design.md docs/plans/2026-02-23-design-critic-skill-plan.md
git commit -m "docs: add design-critic skill design doc and implementation plan"
```
...
|------|-------------|-------|-----------|
| 1 | Scaffold SKILL.md with frontmatter | 1 create | 2 min |
| 2 | Operation modes + execution flow | 1 modify | 3 min |
| 3 | 5-pillar evaluation framework | 1 modify | 5 min |
| 4 | Anti-patterns + premium patterns + hard rules | 1 modify | 4 min |
| 5 | Report template + cross-skill integration | 1 modify | 4 min |
| 6 | Verification + final commit | 0 | 2 min |

**Total**: 6 tasks, 1 file, 6 commits.
