...
export default function SessionListPreview() {
  const sessions = sampleData.sampleSessions as EvalSession[]

  const [statusFilter, setStatusFilter] = useState<SessionStatusFilter>('todos')
  const [treatmentFilter, setTreatmentFilter] = useState<TreatmentFilter>('todos')
  ...
  const filteredSessions = sessions.filter((s) => {
    // Status filter
    if (statusFilter === 'concluidos' && s.completedCount !== s.evaluationCount) return false
    if (statusFilter === 'progresso' && s.completedCount === s.evaluationCount) return false

    // Treatment filter
    if (treatmentFilter !== 'todos' && !s.treatmentTypes.includes(treatmentFilter)) return false
          ...
          <div className="space-y-4">
            {paginatedSessions.map((session, index) => {
              ...
              const card = (
                ...
                >
                  {/* Top row: patient name + age + treatment chips */}
                  ...
                  {/* Middle: tooth count + tooth badges + date */}
              ...
              if (isFirst) {
                return (
                  <div key={session.session_id} className="ai-shimmer-border rounded-xl">
                    {card}
                  </div>
                )
              }
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 space-y-4">
      ...
      <div className="text-center space-y-1">
        <p className="font-medium text-foreground">Nenhuma avaliacao encontrada</p>
