...
export async function generateProtocolPDF(data: PDFData): Promise<void> {
  ...
  if (data.clinicName) {
    ...
    addText('DentAI Pro - Protocolo de Restauracao Estetica', headerTextStartX, 20, { fontSize: 9, color: [191, 219, 254] });
  } else {
    addText('DentAI Pro', headerTextStartX, 14, { fontSize: 20, fontStyle: 'bold', color: [255, 255, 255] });
  ...
  pdf.setFontSize(7);
  pdf.setFont('helvetica', 'normal');
  pdf.text('Gerado por DentAI Pro', pageWidth / 2, footerY + 2, { align: 'center' });
